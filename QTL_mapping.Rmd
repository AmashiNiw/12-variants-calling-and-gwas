...
Many phenotypes (traits) 
are quantitative in nature. Variation in such quantitative traits is often due to the effects of multiple
genetic loci as well as environmental factors. Knowledge of the number, locations,
effects, and identities of such genetic loci, called **quantitative trait loci
(QTL)**, can lead to new biological insights.


Studies identifying QTL assemble or create a genetically diverse population.
Then we associate genetic variation with phenotypic variation in the study
population. Regions of the genome that show convincing evidence of association
with the phenotypes are flagged as QTL.

QTL mapping is to obtain phenotype data on a number
of backcross or intercross progeny and then identify regions in the genome
where genotype is associated with the phenotype. 
However, the genotype is
not observed at every possible position along the chromosomes, but only at a
set of discrete landmarks called **genetic markers**. 
...
as sex, cross direction, and environmental conditions such as diet. 
 The **genotype data** consists of a set of
genetic markers spanning the genome. 
The **genetic map** specifies the markers locations on the chromosomes in terms
of genetic distance. 

We will use `qtl` package in R for QTL mapping.
```{r}
library(qtl)
```

## Breading experiments 

### Backcross experiment

![Backcross experiment](backcross_experiment.png)
The two inbred (bred from closely related ones) strains, A and B are crossed to produce
F1 generation receiving a
single chromosome from each parent. All individuals in F are genetically identical. If
we cross an individual from the F1 generation “back” to one of the parental strains, we obtain a population exhibiting genetic variation.
The **backcross** individuals receive an intact A chromosome from their A parent. 


### Intercross experiment

![Schematic of autosomal representation of intercross experiment](intercross_experiment.png)
The two inbred strains, A and B, are crossed to obtain F1 generation. By crossing two individuals in the F1 generation (or by selfing
when possible), we create genetic variation in the resulting F2 population. The three
possible genotypes AA, AB, and BB appear in a 1:2:1 ratio.
...
## Salt-induced hypertension data in the mouse.


```{r}
# Data import to an object of class cross
