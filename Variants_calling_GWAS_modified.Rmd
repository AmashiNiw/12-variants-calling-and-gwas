...
# Part I: Generating DNA sequencing data

## Introduction: Variant calling from DNA-Seq data
...
The first-generation sequencing emerged in 1970s when Allan Maxam and Walter Gilbert developed a **chemical sequencing method** for sequencing, followed by Frederick Sanger who developed the chain-termination method (also known as **Sanger sequencing**). Both methods were used in shotgun sequencing, which involves breaking DNA into fragments and sequencing each fragment individually. The first step in Sanger sequencing is that of PCR. However, sample DNA is divided into four reaction tubes fluorescently labelled by di-deoxynucleotide triphospates: ddATP, ddGTP, ddCTP, and ddTTP. The synthesis terminates in DNA fragments in individual nucleotides which are then separated by size and identified using gel electrophoresis. The DNA bands are visualized by autoradiography, allowing the nucleotide sequence to be read directly from the X-ray film.

### Next-generation sequencing (NGS)

Next-generation sequencing (NGS) is a massively parallel DNA sequencing technology that can analyze hundreds of thousands of DNA fragments simultaneously. Compared to earlier sequencing technologies, NGS provides sequencing information on multiple fragments simultaneously. DNA is converted into single strands (denatured), tagged with fluorescent markers, and then allowed to hybridize with known libraries or reference genomes. Fluorescent signals detected by the sensors are used to identify the DNA sequence.   

![NGS workflow](NGS_workflow.jpg)

NGS refers to a class of technologies that sequence millions of short DNA fragments in parallel, with a relatively low cost. The length and number of the reads differ based on the technology. Currently, there are three major commercially available platforms/ technologies for NGS: (1) Illumina, (2) Roche, and (3) Life Technologies. The underlying chemistry and technique used in each platform is unique and affects the output.


NGS enables a wide variety of applications, allowing researchers to ask virtually any question related to the genome, transcriptome, or epigenome of any organism. These methods differ primarily by how the DNA or RNA samples are prepared and the data analysis options used. The technology is used to determine the order of nucleotides in entire genomes or targeted regions of DNA or RNA. 

### Third-generation sequencing

Third generation sequencing (TGS) is a recent technique that addresses drawbacks of NGS, such as the need for long reads and poor resolution (i.e., poor and weak reads). The foundation of TGS emerged when DNA polymerase was used to obtain a sequence of single DNA molecules, which involves (i) direct imaging of individual DNA molecules using advanced microscopy techniques and (ii) nanopore sequencing technique in which a single molecule of DNA is threaded through a nanopore (a pore of nanoscale). The DNA's passage through the pore creates signals that can be converted to read the DNA sequence
...
# Part II: Understanding sequencing data quality
...
## Sequence file formats: FASTA, FASTQ, and FastQC  
...
# Part III: From reads to a genome
